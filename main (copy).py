...
def main():
    ...
    This function sets up an agent using the Llama 3 model with the ChatGroq API.
    It provides a text-based interface for users to generate locations for necromunda, generate scenarios for necromunda, make dice rolls and look up rules and references
    ...
    2. Display introductory text about the CrewAI agents.
    3. Create and configure four AI agents:
        - Location_Generation_Agent: Generates a location and details for the game of Necromunda.
        - Rules_Reference_Agent: Evaluates the questions from the user and looks up a rules reference from the rules of Necromunda.
        - Combat_Simulation Agent: Based on details such as the scenario, locations, factions playing and gangers, will help run each round of Necromunda combat.
        - Gang_Manager_agent: An agent that is designed to help manage the stats, status, inventory, skills and notes for each gang member of a gang in the game of Necromunda.
    4. Prompt the user to select an action.
